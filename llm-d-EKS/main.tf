...
module "eks" {
  ...
  # Pin a stable version that supports the inputs below
  version = "20.31.4"
  ...
  # Public API (simpler from a bastion). You can flip later.
  ...
  eks_managed_node_groups = {
    cpu = {
      ...
      # For Kubernetes 1.33, AL2023 is required (AL2 not available)
    ...
    gpu = local.gpu_enabled ? {
      ...
      # AL2023 accelerated image with NVIDIA
...
# --- Tag existing subnets so LBs land correctly ---
